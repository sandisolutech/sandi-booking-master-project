...
import { createTimeRound } from "@/app/admin/settings/time-rounds/actions" // Import the action
...
export function AddRoundDialog({ open, onOpenChange }: AddRoundDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await createTimeRound({ 
      name: roundName, 
      startTime, 
      endTime, 
      bookingLimit 
    ...
    if (result.success) {
      toast({
        title: "Success!",
        description: `Time round "${result.data?.name}" created.`,
      })
      onOpenChange(false)
      // Reset form
      setRoundName("")
      ...
      toast({
        title: "Error",
        description: result.message || "Failed to create time round.",
        variant: "destructive",
      })
    }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[400px]">
        <DialogHeader>
          <DialogTitle>Add Time Round</DialogTitle>
          <DialogDescription>Create a new time slot that can be booked for any room.</DialogDescription>
        </DialogHeader>
          ...
          {startTime && endTime && (
            <div className="bg-blue-50 p-3 rounded-lg">
              <p className="text-sm text-blue-800">
                <strong>Preview:</strong> This time slot will be available for booking across all active rooms with a limit of {bookingLimit} booking{bookingLimit !== 1 ? 's' : ''} per slot.
              </p>
            </div>
          )}
