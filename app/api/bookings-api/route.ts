  ...
  try {
    ...
    if (!secretKey) {
      return NextResponse.json(
        { success: false, error: "Unauthorized. Valid secret key required." },
        { status: 401 }
      )
    }
    ...
    console.log("📊 Query Parameters:", {
      customKey, value, companyId, roomId, status, search, startDate, endDate, page, pageSize
    })

    // Execute queries with conditional logic using simple approach
    let result: any
    let countResult: any

    // Helper function to execute query based on filters
    if (!customKey && !companyId && !roomId && !status && !search && !startDate && !endDate) {
      // No filters - simple query
      result = await sql`
        SELECT
          b.id, b.selected_date, b.selected_room_id, r.name AS room_name,
          b.selected_time_slot_id, tr.start_time AS time_slot_start_time,
          tr.end_time AS time_slot_end_time, b.custom_company_name,
          b.agreed_to_terms, b.status, b.link_id, b.created_at, b.updated_at,
          b.booking_number, b.company_id, c.name AS company_name
        FROM bookings b
        LEFT JOIN rooms r ON b.selected_room_id = r.id
        LEFT JOIN time_rounds tr ON b.selected_time_slot_id = tr.id
        LEFT JOIN companies c ON b.company_id = c.id
        ORDER BY b.selected_date DESC, tr.start_time ASC
        LIMIT ${pageSize} OFFSET ${offset}
      `
      countResult = await sql`SELECT COUNT(*) as total FROM bookings`
    } else {
      // With filters - build query dynamically using sql.unsafe if available
      let conditions = []
      ...
      const countQuery = `
        ...
        WHERE 1=1${whereClause}
      `
      console.log("Executing dynamic query with conditions:", mainQuery)
      try {
        // Use sql.unsafe with template literal wrapper for dynamic queries
        result = await sql`${(sql as any).unsafe(mainQuery)}`
        countResult = await sql`${(sql as any).unsafe(countQuery)}`
      } catch (error) {
    ...
    const totalCount = Array.isArray(result) ? (countResult[0]?.total ?? 0) : ((countResult as any).rows?.[0]?.total ?? 0)
    ...
    return NextResponse.json(
      { code: 1000, message: "Failed to fetch bookings", error: err?.message ?? "Unknown error" , data:null},
      { status: 500 }
    )
  }
}
